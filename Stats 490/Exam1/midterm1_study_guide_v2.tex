...
\newtcolorbox{calcbox}{colback=gray!8, colframe=gray!50, boxrule=0.3pt, left=3pt, right=3pt, top=2pt, bottom=2pt, fontupper=\small}
\newtcolorbox{keybox}{colback=blue!5, colframe=blue!40, boxrule=0.3pt, left=3pt, right=3pt, top=2pt, bottom=2pt, fontupper=\small}
...
{\footnotesize 0. Table of contents: 
1) Foundations\quad
2) One-way ANOVA\quad
3) Multiple Comparisons\quad
4) Model Adequacy\quad
5) Random Effects\quad
6) RCBD\quad
7) Power\quad
8) Non-parametrics\quad
9) Design Principles\quad
10) Distributions}
...
\textit{One-sample $t$}: SRS from population; errors $X-\mu$ i.i.d; population approximately normal or $n$ large.\\
\textit{Two-sample pooled $t$}: two independent SRSs; both populations approximately normal; equal variances ($\sigma_1^2=\sigma_2^2$); can check via SD ratio, F-test, Bartlett, or Levene.\\
\textit{Two-sample Welch $t$}: two independent SRSs; normal or large $n$; variances may differ, but distributions should have similar shape.\\
\textit{Paired $t$}: pairs form an SRS of differences; differences $d_i$ approximately normal or $n$ large; pairs independent of each other.
...
\textbf{Theory}: every $t$ statistic has the form
\[
t = \frac{\text{signal}}{\text{noise}} = \frac{\text{estimated effect}}{\text{standard error under }H_0},
\]
e.g.\ $(\bar{x}-\mu_0)/(s/\sqrt{n})$ or $(\bar{x}_1-\bar{x}_2)/SE$. Under $H_0$ we plug the random sample SD $s$ in place of $\sigma$, which makes the standardized statistic follow a $t$ distribution instead of a normal one.
...
\textbf{Satterthwaite}: Approximation for degrees of freedom in Welch's t-test: $\Delta = \frac{(s_1^2/n_1 + s_2^2/n_2)^2}{\frac{(s_1^2/n_1)^2}{n_1-1} + \frac{(s_2^2/n_2)^2}{n_2-1}}$.
...
\textbf{Fixed-effects one-factor ANOVA assumptions:}
\begin{itemize}
  \item Errors $\varepsilon_{ij}$ are independent (ensured by randomization).
  \item $\varepsilon_{ij}\sim N(0,\sigma^2)$ for all $i,j$ (same distribution and variance across groups).
  \item The factor is correctly specified (no hidden time trends or other structure left in the errors).
\end{itemize}
...
Under $H_0:\mu_1=\cdots=\mu_a$,
\[
E[MS_{\text{Treat}}]=\sigma^2,\qquad E[MS_E]=\sigma^2,
\]
so the $F$ statistic $MS_{\text{Treat}}/MS_E$ should be near 1. Under $H_a$ (at least one $\mu_i$ differs),
\[
E[MS_{\text{Treat}}]=\sigma^2 + \frac{n\sum_{i=1}^a\tau_i^2}{a-1}>\sigma^2,
\]
so $F$ tends to be larger than 1. Intuitively, $F$ is ``between-group variance estimate / within-group variance estimate''.\\[2pt]
\textit{Robustness}: the ANOVA $F$-test is fairly robust to mild non-normality, especially for balanced designs and moderate sample sizes; violations of equal variances are usually more serious than modest departures from normality.
...
The definition of the ``family'' depends on context: it might be all pairwise comparisons, a small set of pre-planned contrasts, or all contrasts you decide to examine after looking at the data.
...
\textbf{Fisher LSD:}
\begin{itemize}
  \item Requires a significant overall ANOVA $F$ first.
  \item Assumes the same ANOVA assumptions (normality, equal variance).
  \item Does \emph{not} control familywise error well when many comparisons are made; relatively liberal.
\end{itemize}

\textbf{Bonferroni:}
\begin{itemize}
  \item No special assumptions beyond those of the underlying $t$-tests.
  \item Uses $\alpha/K$ per test (or $1-\alpha/K$ CIs) to keep FW error $\le\alpha$ for any chosen family.
\end{itemize}

\textbf{Tukey HSD:}
\begin{itemize}
  \item Based on the Studentized range distribution (max--min of sample means).
  \item Assumes equal variances and works best with balanced or nearly balanced designs.
  \item Designed for all pairwise comparisons with strong FW error control.
\end{itemize}

\textbf{Dunnett:}
\begin{itemize}
  \item Similar spirit to Tukey but conditional on a single control mean.
  \item Uses the joint distribution of treatment–control differences.
  \item Only valid for comparing each treatment to one control (not all pairwise).
\end{itemize}

\textbf{Scheffé:}
\begin{itemize}
  \item Uses a scaled $F$-statistic valid for \emph{any} contrast, including those chosen after seeing the data.
  \item Guarantees FW error $\le\alpha$ over all contrasts, but is very conservative, especially for simple pairwise differences.
\end{itemize}
...
\textbf{Theory (delta method idea):} if $\mathrm{Var}(Y)\approx g(\mu)$ and $h'(\mu)\propto 1/\sqrt{g(\mu)}$, then $\mathrm{Var}(h(Y))$ is approximately constant, which motivates choices like $h(y)=\sqrt{y}$ when $\mathrm{Var}(Y)\propto\mu$ (Poisson) or $h(y)=\log y$ when $\mathrm{Var}(Y)\propto\mu^2$.\\[2pt]
\textbf{Practical decision rule:}
\begin{itemize}
  \item If variance increases with the mean $\Rightarrow$ try $\sqrt{\cdot}$ or $\log(\cdot)$.
  \item If response is a proportion near 0 or 1 $\Rightarrow$ try $\arcsin\sqrt{\cdot}$.
  \item If transformations fail and residuals show strong non-normality or outliers, consider non-parametric tests instead of $t/F$.
\end{itemize}
...
\textbf{Random one-way effects assumptions:}
\begin{itemize}
  \item Levels $i=1,\ldots,a$ are a random sample from a larger population of levels.
  \item $\tau_i\sim N(0,\sigma_\tau^2)$ i.i.d.
  \item $\varepsilon_{ij}\sim N(0,\sigma^2)$, independent of each other and of all $\tau_i$.
  \item Within each level $i$, the $Y_{ij}$ are exchangeable (same distribution for all $j$ at that level).
\end{itemize}
...
Heuristic derivation: $MS_{\text{Trt}}$ averages squared deviations of group means from the grand mean. Each group mean has variance $\sigma^2/n$ from noise plus $\sigma_\tau^2$ from the random effects, so
\[
E[MS_{\text{Trt}}] = \sigma^2 + n\sigma_\tau^2,\qquad E[MS_E]=\sigma^2.
\]
The $F$-test $MS_{\text{Trt}}/MS_E$ therefore tests $H_0:\sigma_\tau^2=0$ (no between-level variance).\\[2pt]
\textbf{ICC interpretation:} $\rho = \sigma_\tau^2/(\sigma_\tau^2+\sigma^2)$ is the correlation between two units from the same level, and the fraction of total variance explained by ``which level you are in''. If $\rho\approx0$, grouping is not important; if $\rho\approx1$, observations within a level are almost identical.
...
\textbf{RCBD assumptions:}
\begin{itemize}
  \item Within each treatment-block cell, errors $\varepsilon_{ij}$ are independent across all $i,j$.
  \item Errors are approximately normal with common variance $\sigma^2$.
  \item Additive model: no treatment$\times$block interaction; the difference between any two treatments is the same in every block.
\end{itemize}
...
For mixed RCBD with random blocks,
\[
Y_{ij} = \mu + \tau_i + \beta_j + \varepsilon_{ij},\quad \beta_j\sim N(0,\sigma_\beta^2),
\]
with treatments fixed. Expected mean squares:
\[
E[MS_{\text{Trt}}] = \sigma^2 + \frac{b\sum\tau_i^2}{a-1},\quad
E[MS_{\text{Block}}] = \sigma^2 + a\sigma_\beta^2,\quad
E[MS_E] = \sigma^2.
\]
If a real treatment$\times$block interaction exists but is not modeled, its variation is absorbed into $MS_E$, inflating the error term and reducing power for both treatment and block $F$-tests (higher Type II error).
...
\textbf{Parametric vs non-parametric assumptions:}
\begin{itemize}
  \item Wilcoxon signed-rank vs paired $t$: requires that the distribution of differences is symmetric about the median; no normality needed. More robust to outliers; slightly less powerful than $t$ if data truly normal.
  \item Mann--Whitney vs two-sample $t$: independent samples, similar shape distributions; tests a shift in location (e.g.\ $P(X>Y)\neq 0.5$). If shapes differ, it is closer to testing ``stochastic ordering''.
  \item Kruskal--Wallis vs ANOVA $F$: independent samples and continuous distributions; null is equal distributions across groups, and the test is based on rank sums. Under true normality with equal variances, $F$ is more powerful; under heavy tails or strong skew, Kruskal--Wallis $H$ can be preferable.
\end{itemize}
