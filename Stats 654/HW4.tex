...
\]
Since $x_{-i} = y_{-i}$, we can write both conditionals with the same
conditioning argument and use the definition of conditional probability:
\[
\pi(y_i \mid x_{-i})
...
= \frac{\pi(x)}{\pi(y_{-i})}.
\]
Again using $x_{-i} = y_{-i}$, we obtain
\[
...
\[
\pi(x) P(x,y)
= \pi(x)\,\frac{1}{d}\,\pi(y_i \mid x_{-i})
= \frac{1}{d} \,\pi(x)\,\frac{\pi(y)}{\pi(x_{-i})},
\]
and
\[
\pi(y) P(y,x)
= \pi(y)\,\frac{1}{d}\,\pi(x_i \mid y_{-i})
= \frac{1}{d} \,\pi(y)\,\frac{\pi(x)}{\pi(x_{-i})}.
\]
...
P =
\begin{pmatrix}
\frac{1+\rho}{2} & \frac{1-\rho}{4} & \frac{1-\rho}{4} & 0 \\[4pt]
\frac{1+\rho}{4} & \frac{1-\rho}{2} & 0 & \frac{1+\rho}{4} \\[4pt]
\frac{1+\rho}{4} & 0 & \frac{1-\rho}{2} & \frac{1+\rho}{4} \\[4pt]
0 & \frac{1-\rho}{4} & \frac{1-\rho}{4} & \frac{1+\rho}{2}
\end{pmatrix}.
\]
...
\begin{itemize}
    ...
    iterations from the chosen starting point, so the sample path is flat
    at $+1$ and the histogram has all its mass at $+1$.
\end{itemize}
...
square. Expand the two quadratic terms:
\[
\|x_i - a_{i-1}x_{i-1}\|^2
= x_i^\top x_i - 2 x_i^\top a_{i-1}x_{i-1} + \|a_{i-1}x_{i-1}\|^2,
\]
\[
\|x_{i+1} - a_i x_i\|^2
= x_{i+1}^\top x_{i+1}
...
+ x_i^\top a_i^2 x_i.
\]
Discarding terms that do not depend on $x_i$, we obtain that the exponent is
\[
-\frac{1}{2}
\left[
...
+ \text{const},
\]
where ``const'' denotes terms not depending on $x_i$. Define
\[
\Lambda_i
= \left(
    \frac{1}{\sigma_{i-1}^2}
...
= \frac{a_{i-1}}{\sigma_{i-1}^2} x_{i-1}
  + \frac{a_i}{\sigma_i^2} x_{i+1}.
\]
Then the exponent can be written as
\[
...
= \frac{
    ...
    \frac{1}{\sigma_{i-1}^2} + \frac{a_i^2}{\sigma_i^2}
},
\]
...
\alpha(i,j) = \min\!\left(1,\frac{w_j}{w_i}\right) = 1.
\]
Therefore, for $j<i$,
\[
...
\]
Rewriting $1 - \sum_{j<i} p_j - p_i$ as $\sum_{j>i} p_j$, we obtain
\[
U(i,i)
...
P = U + R,
\]
where $R$ is rank one, $U$ is upper triangular (since $U(i,j)=0$ for $j<i$), and
the diagonal entries of $U$ are
\[
...
eigenvalue is shifted by the amount $u^{\top}u_n$.

Because $P$ is a Markov transition matrix, it must have eigenvalue $1$ with
(right) eigenvector equal to the stationary distribution (up to scaling).
Hence the shifted eigenvalue
\[
\lambda_n(U) + u^{\top}u_n
\]
must be equal to $1$. The remaining eigenvalues of $P$ are exactly the
other diagonal entries of $U$:
\[
\lambda_i(P) = U_{ii},\qquad i = 1,\dots,n-1,
\]
up to a relabeling of indices. In particular, the full multiset of eigenvalues
of $P$ can be described as
\[
\{1\} \,\cup\, \{U_{11},\dots,U_{nn}\}\setminus\{\text{one diagonal entry
that is shifted to } 1\}.
\]
Equivalently, we may write: the eigenvalues of $P$ consist of $1$ together
with the diagonal entries of $U$, except for one diagonal entry that has been
shifted to $1$ by the rank-one perturbation.
    \end{solution}
    \item Calculate the spectral gap of this independent Metropolis--Hastings chain.
    \begin{solution}
From part (4), the eigenvalues of $P$ consist of
\[
1 \quad\text{and}\quad U_{11},\dots,U_{nn},
\]
with exactly one of the diagonal entries $U_{ii}$ shifted to $1$ by the
rank--one perturbation. In particular, all \emph{nontrivial} eigenvalues
(those different from $1$) are among the diagonal entries of $U$:
\[
\{\lambda : \lambda \neq 1\} \subset \{U_{11},\dots,U_{nn}\}.
\]
...
\[
\gamma
= 1 - \max\{|\lambda| : \lambda \text{ is an eigenvalue of } P,\ \lambda \neq 1\}.
\]
Using the description above and the fact that all nontrivial eigenvalues are
nonnegative, this becomes
\[
\gamma
= 1 - \max\{\lambda : \lambda \neq 1\}
= 1 - \max_{i : U_{ii} \neq 1} U_{ii}.
...
\]
where the maximum is taken over all indices $i$ such that $U_{ii}$ is a
nontrivial eigenvalue (i.e.\ not shifted to $1$). In terms of the proposal
probabilities $(p_j)$ and weights $(w_j)$, this expression gives the spectral
gap of the independent Metropolis--Hastings chain.
    \end{solution}
\end{enumerate}
