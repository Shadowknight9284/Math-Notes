...
\begin{exercise}[7]
      ...
      We can approach this by using the 
        \begin{itemize}
          ...
          \begin{align*}
            ...
            \psi_3 &= \theta_3 + \phi_1\psi_2 + \phi_2\psi_1 + \phi_3\psi_0 = 0.622\\
            \psi_4 &= \phi_1\psi_3 + \phi_2\psi_2 + \phi_3\psi_1 = -0.682\\
            \psi_n &= \phi_1\psi_{n-1} + \phi_2\psi_{n-2} + \phi_3\psi_{n-3} \quad \text{for } n > 4
          \end{align*}
          I have omitted the full calculations for brevity, but following this method we can compute the ACF values up to lag 12.\\
          Similarly, to calculate $\rho(h)$ we can divide $\gamma(h)$ by $\gamma(0)$.\\
          I will be omitting the full calculations for brevity as well.
        \end{itemize}
    \end{solution}
  \item [(b)] Simulate a series of length $T=250$, give the time series plot.
    \begin{solution}
        \begin{center}
          \includegraphics[width=0.8\textwidth]{img/7b_img.png}
        \end{center}
    \end{solution}
  \item [(c)] Compare the true ACF plot (plot what you obtained in Part (a))
    ...
    \begin{solution}
        \begin{center}
          \includegraphics[width=0.8\textwidth]{img/7c_img.png}
        \end{center}
        The graph differences between the true and observed realization of the plots are quite significant. The main note is that the sample ACF has much more variability due to the finite sample size of 250. The true ACF is smooth and follows the theoretical values closely, while the sample ACF shows fluctuations around the true values. This is expected as the sample ACF is an estimate based on a finite number of observations, leading to sampling variability.
    \end{solution}
  \end{enumerate}
\end{exercise}
...
\begin{exercise}[9]
  ...
  \item If $\phi_1,\ldots,\phi_p$ are the AR coefficients for $Y_t$,
    ...
    $|h|>q$.
    \begin{solution}
      Since $\{Y_t\}$ and $\{X_t\}$ have the same autocovariance function $\gamma(h)$, we can express the autocovariance function of $\{W_t\}$ as follows:
      \begin{align*}
        \gamma_W(h) &= \cov(\phi(B)X_t, \phi(B)X_{t+h}) \\
        &= \sum_{i,j=0}^{p} \phi_i \phi_j\gamma(h+j-i)
      \end{align*}
      Now since $\{Y_t\}$ is an ARMA($p,q$) process, we have $\phi(B)Y_t = \theta(B)Z_t$ where $\theta(B)$ is a polynomial of degree $q$.\\
      Thus define
      \begin{align*}
        \gamma_{W'}(h) &:= \cov(\phi(B)Y_t, \phi(B)Y_{t+h}) \\
        &= \cov(\theta(B)Z_t, \theta(B)Z_{t+h}) \\
        &= 0 \quad \text{for } |h| > q
      \end{align*}
      Since $\{X_t\}$ and $\{Y_t\}$ have the same autocovariance function, we have $\gamma_W(h) = \gamma_{W'}(h)$. Therefore, we conclude that $\gamma_W(h) = 0$ for $|h| > q$.
    \end{solution}
